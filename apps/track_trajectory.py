...
# Optional ESN reference generator (adaptive にも対応)
try:  # pragma: no cover - optional dependency
    from esn_ref_trajectory import (
        load_model as load_esn_model,
        ReferenceGenerator,
        ReferenceGeneratorAdaptive,
    )  # type: ignore
...
def track_motion_trajectory(
    ...
    *,
    ...
    while True:
        ...
        # Reference generation (ESN first if provided)
        if esn_generator is not None and ReferenceGenerator is not None:
            try:
                qdes_esn = esn_generator.step(measured_q=q_meas)
                dqdes = 0.0 if qdes_esn is None else ((qdes_esn - q_meas) / dt_prev if dt_prev > 0 else 0.0)
                qdes = qdes_esn
            except Exception:
                # Fallback to original reference functions if ESN step fails
        ...
        if trained_model is not None:
            ...
            try:
                ...
                if esn_generator is not None:
                    # ESN使用時: ESNのqdesを未来目標として使用 (時間整合不要)
                    qdes_prev = _to_full_vec_at_index(_a_angle(qdes))[jnts]
                    feats.append(qdes_prev)
                    if include_dqdes:
                        dqdes_prev = _to_full_vec_at_index(_a_vel(dqdes))[jnts]
...
def parse() -> argparse.Namespace:
    ...
    # ESN adaptive reference options
    parser.add_argument("--esn-model", type=str, default=None, help="ESN参照生成モデル(.npz)。q->q+1 予測または q->qdes 適応参照 (meta.adaptive_alpha) を使用")
    # NOTE: reset semantics removed from ESN ReferenceGenerator; no CLI flag
...
def main() -> None:
    ...
    # Prepare ESN generator if requested
    esn_generator = None
    if args.esn_model and load_esn_model is not None:
        try:
            esn_path = Path(args.esn_model)
            esn = load_esn_model(esn_path)  # type: ignore
            adaptive_alpha = None
            meta_mode = None
            try:
                data = np.load(esn_path, allow_pickle=True)
                meta = data["meta"].tolist() if hasattr(data["meta"], "tolist") else data["meta"].item()
                adaptive_alpha = meta.get("adaptive_alpha", None)
                meta_mode = meta.get("mode", None)
            except Exception:
                adaptive_alpha = None
                meta_mode = None
            # adaptive_alpha が存在し ReferenceGeneratorAdaptive が利用可能ならそちらを使う
            if adaptive_alpha is not None and ReferenceGeneratorAdaptive is not None:
                esn_generator = ReferenceGeneratorAdaptive(esn, alpha=float(adaptive_alpha))  # type: ignore
                print(f"[INFO] ESN adaptive reference model loaded (alpha={adaptive_alpha} mode={meta_mode}): {args.esn_model}", flush=True)
            elif ReferenceGenerator is not None:
                esn_generator = ReferenceGenerator(esn)  # type: ignore
                print(f"[INFO] ESN reference model loaded (q->q+1 mode): {args.esn_model}", flush=True)
            else:
                print("[WARN] ESN generators unavailable", flush=True)
        except Exception as e:
            print(f"[WARN] Failed to load ESN model '{args.esn_model}': {e}", flush=True)

    # If ESN is active we still need at least one dummy reference file path (first provided) to satisfy parser.
