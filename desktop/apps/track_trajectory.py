...
# Optional ESN reference generator (reset/history 機能は削除済み仕様を前提)
try:  # pragma: no cover - optional dependency
    from esn_ref_trajectory import (
        load_model as load_esn_model,
        ReferenceGenerator,
    )  # type: ignore
    try:
        # Adaptive 用 (q->qdes) も利用可能なら取り込む
        from esn_ref_trajectory import ReferenceGeneratorAdaptive  # type: ignore
    except Exception:  # pragma: no cover
        ReferenceGeneratorAdaptive = None  # type: ignore
except Exception:  # pragma: no cover
    load_esn_model = None  # type: ignore
    ReferenceGenerator = None  # type: ignore
    ReferenceGeneratorAdaptive = None  # type: ignore
...
def main() -> None:
    ...
    # If esn_ref_trajectory import failed earlier at module import time, retry here
    if args.esn_model and (load_esn_model is None or ReferenceGenerator is None):
        try:
            from esn_ref_trajectory import load_model as load_esn_model, ReferenceGenerator  # type: ignore
            try:
                from esn_ref_trajectory import ReferenceGeneratorAdaptive  # type: ignore
            except Exception:
                ReferenceGeneratorAdaptive = None  # type: ignore
            print(f"[INFO] esn_ref_trajectory imported from {_DESKTOP_APPS}", flush=True)
        except Exception as e:
            print(f"[WARN] esn_ref_trajectory import retry failed: {e}", flush=True)
    if args.esn_model and load_esn_model is not None and ReferenceGenerator is not None:
        try:
            _esn_path = Path(args.esn_model)
            esn = None  # type: ignore
            meta = None
            # 旧フォーマット(meta_strのみ) のインライン変換試行
            try:
                _raw = np.load(_esn_path, allow_pickle=True)
                if ('meta' not in _raw.files) and ('meta_str' in _raw.files):
                    meta_raw = _raw['meta_str']
                    try:
                        meta_eval = eval(meta_raw) if isinstance(meta_raw, str) else eval(meta_raw.tolist())  # noqa: S307
                    except Exception:
                        meta_eval = {'exact_matlab': True, 'mode': 'matlab_exact'}
                    matlab_keys = {"WIn","WFb","WOut","W_data","W_indices","W_indptr","W_shape","P"}
                    if matlab_keys.issubset(set(_raw.files)):
                        from affetto_nn_ctrl.esn_matlab_exact import MatlabExactESN as _M  # type: ignore
                        converted_dict = {k: _raw[k] for k in matlab_keys}
                        converted_dict['meta'] = meta_eval
                        esn = _M.from_dict(converted_dict)  # type: ignore
                        meta = meta_eval
                        print(f"[INFO] Inline converted legacy meta_str ESN model: {_esn_path}", flush=True)
            except Exception:
                pass
            if esn is None:
                esn = load_esn_model(_esn_path)  # type: ignore
            # meta再取得（標準保存フォーマット or 変換後）
            if meta is None:
                try:
                    data_npz = np.load(_esn_path, allow_pickle=True)
                    meta_raw = data_npz.get('meta', None)
                    if meta_raw is not None:
                        try:
                            meta = meta_raw.tolist() if hasattr(meta_raw, 'tolist') else (meta_raw.item() if hasattr(meta_raw, 'item') else meta_raw)
                        except Exception:
                            meta = meta_raw
                    elif 'meta_str' in getattr(data_npz, 'files', []):
                        # ここには通常来ない（上で処理済）が念のため
                        meta_raw2 = data_npz['meta_str']
                        try:
                            meta = eval(meta_raw2) if isinstance(meta_raw2, str) else eval(meta_raw2.tolist())  # noqa: S307
                        except Exception:
                            meta = {'exact_matlab': True, 'mode': 'matlab_exact'}
                except Exception:
                    meta = None
            alpha = 1.0
            mode = None
            if meta:
                mode = meta.get('mode') or ('matlab_exact' if meta.get('exact_matlab') else None)
                alpha = float(meta.get('adaptive_alpha', meta.get('alpha', 1.0)))
            adaptive_modes = {"adaptive", "matlab_single", "matlab_exact", "matlab_strict"}
            if (ReferenceGeneratorAdaptive is not None and (
                (mode in adaptive_modes) or (meta and ('adaptive_alpha' in meta or 'alpha' in meta))
            )):
                try:
                    esn_generator = ReferenceGeneratorAdaptive(esn, alpha=alpha)  # type: ignore
                    print(f"[INFO] ESN adaptive generator initialized (mode={mode}, alpha={alpha})", flush=True)
                except Exception as e:
                    print(f"[WARN] Failed to init ReferenceGeneratorAdaptive: {e}; fallback to basic", flush=True)
                    esn_generator = ReferenceGenerator(esn)  # type: ignore
            else:
                esn_generator = ReferenceGenerator(esn)  # type: ignore
                print("[INFO] ESN generator (q->q+1) initialized", flush=True)
            # dt 同期 (meta.dt 優先)
            try:
                if meta and 'dt' in meta and meta['dt'] is not None:
                    dt_meta = float(meta['dt'])
                    desired_ms = dt_meta * 1000.0
                    if abs(interval_ms_effective - desired_ms) > 1e-9:
                        print(f"[INFO] Overriding loop interval to match ESN dt: {desired_ms:.3f} ms (was {interval_ms_effective:.3f})", flush=True)
                        interval_ms_effective = desired_ms
            except Exception:
                pass
        except Exception as e:
            print(f"[WARN] Failed to load ESN model '{args.esn_model}': {e}", flush=True)
            print("[INFO] To debug: ensure 'desktop/apps/esn_ref_trajectory.py' is on sys.path or install the esn utility package.", flush=True)
