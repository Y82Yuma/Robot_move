#!/usr/bin/env python
#wsl bash -lc "uv run python apps/train_model.py -v ../20241203T144115/step/sync/fast -j 5 --train-size 0.25 --seed 42 -m apps/config/model.toml -a preview-ref -s std -r mlp.option-list --label test"
"""MixAllでpreviewref 0~10で学習するコマンドは以下
uv run python apps/train_model.py -vv \
  data/myrobot/hirai/csv/20250914/step/fast \
  data/myrobot/hirai/csv/20250914/step/middle \
  data/myrobot/hirai/csv/20250914/step/slow \
  data/myrobot/hirai/csv/20250914/trapezoid/fast \
  data/myrobot/hirai/csv/20250914/trapezoid/middle \
  data/myrobot/hirai/csv/20250914/trapezoid/slow \
...
def run(
    ...
    *,
    ...
    # Override include_tension if user provided option
    if include_tension is not None:
        try:
            adapter.params.include_tension = include_tension
            event_logger().info("Adapter include_tension overridden: %s", include_tension)
        except Exception:
            event_logger().warning("Unable to override adapter.include_tension; adapter may not support it")
...
def parse() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Optionally override adapter.include_tension (True/False). If omitted, use value from model.toml.",
    )
    # Output
    parser.add_argument(
        "-o",
