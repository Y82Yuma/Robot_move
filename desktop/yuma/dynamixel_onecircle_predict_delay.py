...
import time
from model_new_delay import ESN, Tikhonov, dynamic_target_angle_circular_time, animate_esn_trajectory   # model.pyのESNとTikhonovクラスをインポート
import os
...
steps = int(teaching_trajectories.shape[0]*2)  # ティーチングデータのステップ数を利用
...
esn.Output.Wout = np.load(os.path.join(data_directory,"esn_weights_onecircle_delay.npy"))
...
for pos in test_initial_positions_angles:
    ...
    esn_predicted_angle_seq , esn_torque_seq, esn_real_angle_seq, esn_error_seq = esn.trajectory_delay_angle(pos, steps, KP, KD, dt, tau)
...
axs[0].plot(teaching_trajectories[:, 0], teaching_trajectories[:, 1], color='blue', alpha=0.7)
axs[0].scatter(teaching_trajectories[0, 0], teaching_trajectories[0, 1], color='green', marker='o', label="Start")  # 初期位置
...
# 3. ESNの現在角度1の時間変化
axs[3].set_title("ESN Current Angle 1 over Time")
for idx, real_traj in enumerate(esn_real_trajectories):
    axs[3].plot(real_traj[:, 1], color=esn_colors[idx], alpha=0.7, linestyle=":", label=f"Current Angle 1 - Traj {idx+1}")
axs[3].set_xlabel("Time Step")
axs[3].set_ylabel("Current Angle 1")
