...
def extract_data(
    ...
    *,
    ...
    # Try to select the requested keys directly. If they are not present
    # (e.g. CSV uses unindexed column names like 'q' instead of 'q5'),
    # fall back to base names (strip trailing digits) and rename them to
    # the requested keys so downstream code sees consistent column names.
    df = dataset.df
    try:
        subset = df.loc[:, keys]
        used_keys = list(keys)
        ...
        # Build a DataFrame by selecting or duplicating columns so that
        # the returned DataFrame has exactly the requested column names
        cols_to_concat: list[pd.Series] = []
        final_names: list[str] = []
        for key in keys:
            # if exact key exists, use it
            if key in df.columns:
                cols_to_concat.append(df[key].copy())
                final_names.append(key)
                continue
            # strip trailing digits to get base name (e.g. q5 -> q)
            base = re.sub(r"\d+$", "", key)
            if base in df.columns:
                # Duplicate base column for each requested key (handles single-DOF CSV)
                cols_to_concat.append(df[base].copy())
                final_names.append(key)
                continue
            # last resort: try to find any column that starts with base
            candidates = [c for c in df.columns if c.startswith(base)]
            if candidates:
                cols_to_concat.append(df[candidates[0]].copy())
                final_names.append(key)
                continue
            # cannot find a column for this key
            raise KeyError(f"None of [{list(keys)}] are in the [columns]")

        subset = pd.concat(cols_to_concat, axis=1)
        subset.columns = final_names
        renamed = True
    ...
    if keys_replace is not None:
        # keys_replace expects original 'keys' ordering; perform rename accordingly
        extracted = extracted.rename(columns=dict(zip(list(keys), keys_replace, strict=True)))
...
class TrainedEsnModel(Generic[DataAdapterParamsType, StatesType, RefsType, InputsType]):
    ...
    def predict(self, X: np.ndarray) -> np.ndarray | Unknown:  # noqa: N803
        if self.scaler is not None:
            # Use transform here; scaler must have been fitted during training.
            X = self.scaler.transform(X)
        return self.model.predict(X)

    def score(self, X: np.ndarray, y: np.ndarray, sample_weight: np.ndarray | None = None) -> float | Unknown:  # noqa: N803
        if self.scaler is not None:
            # Do not refit scaler at scoring time.
            X = self.scaler.transform(X)
        return self.model.score(X, y, sample_weight)
